...
If Chutes sells **compute** and SN13 sells **data**, **Sportstensor (NetUID 41)** sells something even more directly valuable: **alpha in sports betting markets**. It's one of the most interesting subnets to study because: unlike most Bittensor subnets: SN41 **has real USD revenue** flowing back to the ecosystem as TAO buybacks.
...
- 🔐 Understand the concepts of **almanac binding** and **programmatic trade execution** (we go deep in Phase 2)
- 🚀 See why **SN41 is the second subnet** whose miner you'll deploy in Phase 2 GP-1
:::
...
flowchart TB
    subgraph MINER[🧠 Miner Node]
        M1[⚙️ Prediction Model<br/>Stats + ML] --> PRED[📤 Submit prediction<br/>win_prob, spread, total<br/>+ timestamp]
    end

    PRED --> VAL[⚖️ SN41 Validator<br/>records prediction + timestamp]
...
## ⚖️ How Does Validator Scoring Work?
...
Two advanced concepts we'll cover **in full in Phase 2 GP-1**, but it's important you know the names now:
...
When you register in Phase 2, you'll perform the **hotkey ↔ almanac binding** as one of the required steps.
...
This is optional: basic miners just send predictions. But advanced miners can hook predictions → an execution layer that performs real trading on whitelisted platforms. This is the **Trading Strategies** part in Phase 2 GP-1 Unit 7.
...
SN41 is the **first** subnet whose miner we deploy in Phase 2.

➡️ **Phase 2: GP-1 (Guided Project 1): Sportstensor SN41 Mining Guide** will walk you step-by-step through:
...
The concepts we cover at a high level here: **CLV, almanac, programmatic execution**: you'll implement yourself in Phase 2.

:::tip Learning Strategy
Recommendation: **read this unit first until the concepts are clear**. Later in Phase 2 GP-1, the step-by-step execution will be much easier to digest because the theoretical foundation is in place.
:::
...
7. **You'll deploy this in Phase 2 GP-1**: concepts in this unit translate directly.

### ✅ Quick Check

Before moving on to Unit 4 (Ridges), make sure you can answer:
