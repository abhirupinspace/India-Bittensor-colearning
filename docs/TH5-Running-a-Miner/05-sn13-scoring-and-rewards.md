...
:::note Prerequisites
- ✅ Completed [SN13: Scraping Strategy](./sn13-scraping-strategy)
...
:::tip Optimization
**Prioritize data from the last 24 hours.** Set `max_age_hint_minutes` in the config (see Scraping Strategy) so the scraper skips older posts.
...
| Twitter scraper cookie expired, no alert | Volume drops 80% | Set up health check + alerting (interaction layer) |
...
- ✅ S3 upload stable (see S3 Storage)
...
**Next:** [SN13: S3 Storage →](./sn13-s3-storage)
