...
- Have a baseline prediction that validators can score (before improving the strategy in Unit 7)
:::

:::note Prerequisites
- ✅ [Unit 5: Miner Init & Metadata](./05-miner-init-metadata) complete: miner running, validator queries arriving
...
A simple baseline: convert odds to **implied probability** then normalize. Won't beat the market, but it's a safe fallback and starting point.
...
Improve in Unit 7 with ML / feature engineering.
"""
from __future__ import annotations
import logging
from typing import Any
...
Baseline implied-odds + cache + fallback is **enough** to start. In Unit 7 we'll level up with an ML model. **Run first, optimize later.**
:::
