...
Run the checklist below. **All must pass** before continuing to registration.
...
For real mainnet mining, your miner needs to run **24/7 with a reachable public IP**: that means a cloud VPS, not your laptop. This section covers provider choice, base Ubuntu setup, SSH/firewall hardening, and the key security rule: **hotkey on the VPS, coldkey stays offline.**
...
:::tip Recommendation
**Vultr High Frequency 4 vCPU / 8 GB / 256 GB NVMe in Singapore (SGP)**: enough storage for the working set + can upgrade storage addon. Price ~$48/month. Hourly billing, so 2 weeks = under $25.
:::

### Base Ubuntu Setup & Hardening

After the VPS is active, SSH in as root:
...
:::warning Miner Port
The default miner port is **8091**. Can be changed via the `--axon.port` flag, but make sure **the port is open on the VPS firewall**: otherwise validators can't reach your miner and scoring drops. If there's a cloud firewall (e.g., DigitalOcean Cloud Firewall), open it there too.
:::

### Set Up the Wallet on the VPS (Hotkey Only)

Your coldkey & hotkey already exist on your local machine. Now copy the **hotkey** to the VPS: **WITHOUT** the coldkey (security best practice).
...
#### Option B: Create a Brand New Hotkey
...
:::tip SSH Drops During Long Installs?
Use `tmux` or `screen` so a dropped SSH connection doesn't kill your work: `tmux new -s install`.
:::
